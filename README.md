...
Headonly-Reaction 是一个极速的现代 C++20 仅头文件响应式框架，将 React/Vue 风格的数据流引入原生 C++ —— 完美适用于 **UI 数据流**、**游戏逻辑**、**金融服务**、**实时计算**等场景。
...
Headonly-Reaction 支持多线程，具有自动线程安全检测和单线程模式零开销。
...
要手动构建并安装 **Headonly-Reaction** 响应式框架：

```bash
git clone https://github.com/muyuncangcang/Headonly-Reaction.git && cd Headonly-Reaction
cmake -S . -B build
cmake --install build/ --prefix /your/install/path
```

安装完成后，你可以在自己的基于 CMake 的项目中这样使用 Headonly-Reaction：

```cmake
find_package(Headonly-Reaction REQUIRED)
target_link_libraries(your_target PRIVATE Headonly-Reaction)
```
...
你也可以通过 **vcpkg** 安装 Headonly-Reaction，vcpkg 会自动处理依赖和 CMake 集成。

### 安装 Headonly-Reaction

```bash
cd /path/to/vcpkg
./vcpkg install Headonly-Reaction
```
...
### 在 CMakeLists.txt 中使用 Headonly-Reaction

```cmake
find_package(Headonly-Reaction CONFIG REQUIRED)

add_executable(my_app main.cpp)

# 链接 vcpkg 提供的目标
target_link_libraries(my_app PRIVATE Headonly-Reaction::Headonly-Reaction)
```
...
```cpp
#include <Headonly-Reaction/Headonly-Reaction.h>
#include <iostream>
#include <iomanip>
#include <cmath>

int main() {
    using namespace Headonly-Reaction;
...
```cpp
auto a = Headonly-Reaction::var(1);         // int 变量
auto b = Headonly-Reaction::var(3.14);      // double 变量
```
...
```cpp
auto a = Headonly-Reaction::var(1);
auto b = Headonly-Reaction::var(3.14);
auto sum = Headonly-Reaction::calc([=]() {
...
```cpp
auto ds = Headonly-Reaction::calc([](auto aa, auto bb) {
...
```cpp
auto a = Headonly-Reaction::var(1);
auto b = Headonly-Reaction::var(2);
auto result = Headonly-Reaction::expr(a + b * 3);  // 当 'a' 或 'b' 变化时，result 自动更新
```
...
int val = 10;
auto a = Headonly-Reaction::var(1);
auto dds = Headonly-Reaction::action([&val]() {
    val = a();
});
```
...
int val = 10;
auto a = Headonly-Reaction::var(1);
auto dds = Headonly-Reaction::action([&val](auto aa) {
    val = aa;
}, a);
```
...
```cpp
class PersonField : public Headonly-Reaction::FieldBase {
public:
    PersonField(std::string name, int age):
        m_name(Headonly-Reaction::field(name)),
        m_age(Headonly-Reaction::field(age)){}
...
private:
    Headonly-Reaction::Var<std::string> m_name;
    Headonly-Reaction::Var<int> m_age;
};

auto p = Headonly-Reaction::var(PersonField{"Jack", 18});
auto action = Headonly-Reaction::action(
    []() {
...
auto a = Headonly-Reaction::var(1);
auto b = Headonly-Reaction::var(3.14);
auto ds = Headonly-Reaction::calc([]() { return a() + b(); });
auto ds_copy = ds;
...
Headonly-Reaction 框架允许您通过替换计算函数来**重置计算节点**。
...
TEST(TestReset, ReactionTest) {
    auto a = Headonly-Reaction::var(1);
    auto b = Headonly-Reaction::var(std::string{"2"});
    auto ds = Headonly-Reaction::calc([]() { return std::to_string(a()); });
...
`Headonly-Reaction` 框架支持多种触发模式来控制响应式计算的重新求值时机。此示例展示三种模式:
...
在 `Headonly-Reaction` 框架中，用户获取的所有数据源**实际上都是以弱引用形式存在**，其实际内存**由观察者映射管理**。
用户可以手动调用 **close** 方法，这样所有依赖的数据源也会关闭。

```cpp
auto a = Headonly-Reaction::var(1);
auto b = Headonly-Reaction::var(2);
auto dsA = Headonly-Reaction::calc([=]() { return a(); });
auto dsB = Headonly-Reaction::calc([=]() { return dsA() + b(); });
dsA.close(); //dsB 会自动关闭，因为 dsB 依赖 dsA。
EXPECT_FALSE(static_cast<bool>(dsA));
EXPECT_FALSE(static_cast<bool>(dsB));
```

然而，对于用户获取的弱引用生命周期结束的场景，`Headonly-Reaction` 框架为不同场景提供了多种策略。
